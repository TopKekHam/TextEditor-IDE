...
namespace R
{
    ...
    {
        public float line_height; // in pixels.
        public uint texture;
        public float texture_size; // in pixels
        public TTF_Glyph[] glyphs;
        public char[] glyph_lookup_table;
        public stbtt_fontinfo info;
    }


    public class TTF_Fonts
    {

        public static Mesh GenerateTextMesh(TTF_Font font, string text, float size = 16)
        {
            Mesh mesh = new Mesh();
            ...
            //float relative_size = size / font.line_height;
            float pixel_size = 1 / font.texture;

            for (int i = 0; i < text.Length; i++)
            {
                if (text[i] == '\n')
                {
                    padding_y -= (pixel_size * font.line_height * (size / font.line_height));
                    padding_x = 0;
                    continue;
                }
                ...
                for (int j = 0; j < font.glyph_lookup_table.Length; j++)
                {
                ...
                //verts
                {
                    ...
                    float glyph_top = (-glyph.height * size) + padding_y;
                    float glyph_right_side = (glyph.width * size) + padding_x;
                    ...
                    padding_x = glyph_right_side + (pixel_size * size);
                }
